...
const EMAILJS_PUBLIC_KEY="5CfQAtA4uOD5I2SiW";
...
const DMENUS:any[]=[
  ...
  {id:FRK,label:"Fröken Selfoss – 3 Course",base:FRKRATE,cat:"dinner",note:"Flat rate incl. VAT · 15% group discount · no chef service fee",del:false},
  {id:"staff_dinner",label:"Staff / Host Dinner",base:2900,cat:"other",note:"Flat rate incl. VAT · no markup · lodge management only",del:false},
  ...
  {id:"lunch_box",label:"Takeaway Lunch Box",base:3800,cat:"lunch",note:"Fixed flat rate incl. VAT — no additional markup",del:false},
];
const CATS:any={breakfast:"Breakfast",dinner:"Dinner",lunch:"Lunch",other:"Other",custom:"Custom"};

const avat=(b:number)=>b*(1+VAT),afull=(b:number)=>b*(1+VAT)*(1+MKP);
...
function calcDay(day:any,GG:number,GD:string,menus:any[]){
  ...
  const part=day.type==="partial",tr=part?PR:1;
  const mb:any=Object.fromEntries(menus.map(m=>[m.id,m]));
...
function GIn({value,onChange,inh,label,sub}:{value:string,onChange:(v:string)=>void,inh:number,label?:string,sub?:boolean}){
  ...
  return <div>
    ...
    <div style={{position:"relative",display:"flex",alignItems:"center"}}>
      ...
      {ov&&<button onClick={()=>onChange("")} style={{position:"absolute",right:5,background:"none",border:"none",cursor:"pointer",fontSize:12,color:"#f59e0b",padding:0}}>✕</button>}
    </div>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    s.onload=()=>{(window as any).emailjs?.init({publicKey:EMAILJS_PUBLIC_KEY});};
    document.head.appendChild(s);
  },[]);

  const rangeDates=useMemo(()=>genRange(rangeStart,rangeEnd),[rangeStart,rangeEnd]);
  const dates=useMemo(()=>{
    const set=new Set([...rangeDates.filter(d=>!excluded.has(d)),...extraDates]);
    return [...set].sort();
  ...
  const toggleExclude=(d:string)=>setExcluded(prev=>{const n=new Set(prev);n.has(d)?n.delete(d):n.add(d);return n;});
  const addExtra=()=>{if(!newExtra||dates.includes(newExtra))return;setExtraDates(p=>[...p,newExtra].sort());setNewExtra("");};
  ...
  const tots=useMemo(()=>{
    ...
    calcs.forEach((dc:any)=>dc.lines.forEach((l:any)=>{
      ...
      mMap[l.key].days++;mMap[l.key].pax+=l.pax||0;mMap[l.key].base+=l.base;mMap[l.key].vatC+=(l.vatC||0);mMap[l.key].full+=l.full;
    }));
    let can:any=null;
    if(dates.length>0){
  ...
  const mLabel=(m:any)=>FLATIDS.includes(m.id)?`${ISK(m.base)}/pax (flat)`:`est. ${ISK(afull(m.base))}/pax`;
  const Tab=({id,l}:{id:string,l:string})=><button onClick={()=>setTab(id)} style={{padding:"9px 14px",border:"none",cursor:"pointer",fontSize:13,fontWeight:tab===id?700:400,color:tab===id?"#111827":"#6b7280",background:"none",borderBottom:tab===id?"2px solid #111827":"2px solid transparent",whiteSpace:"nowrap"}}>{l}</button>;
  const HolB=({n}:{n:string})=><span style={{marginLeft:6,padding:"1px 5px",borderRadius:3,fontSize:10,background:"#fef3c7",color:"#92400e",fontWeight:700}}>{n} · ×1.9</span>;
...
<script>window.onload=()=>window.print()<\/script></body></html>`;
    ...
    if((window as any).emailjs){
      (window as any).emailjs.send(EMAILJS_SERVICE_ID,EMAILJS_TEMPLATE_ID,{
        quote_ref:QR,client_name:cN||"—",client_email:cE||"—",client_location:cL||"—",
        dates:dates.map(d=>fdS(d)).join(", "),total:ISK(tots.grand),
    ...
    <div style={{fontFamily:"system-ui,sans-serif",maxWidth:980,margin:"0 auto",padding:"20px 16px",color:"#111827"}}>
      ...
      <div style={{background:"#f9fafb",border:"1px solid #e5e7eb",borderRadius:10,padding:"14px 16px",marginBottom:16}}>
        ...
        {rangeDates.length>0&&<div style={{marginBottom:10}}>
          <div style={{fontSize:11,color:"#6b7280",marginBottom:6}}>Uncheck days to exclude from the quote (no charge for excluded days):</div>
        ...
        {extraDates.length>0&&<div style={{marginBottom:8}}>
          ...
          <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
            {extraDates.map(d=><span key={d} style={{display:"flex",alignItems:"center",gap:4,padding:"3px 8px",borderRadius:5,border:"1px solid #bfdbfe",fontSize:11,background:"#eff6ff",color:"#1e40af"}}>
              {fdS(d)}<button onClick={()=>removeExtra(d)} style={{background:"none",border:"none",cursor:"pointer",fontSize:11,color:"#1e40af",padding:0,lineHeight:1}}>✕</button>
            </span>)}
          </div>
        </div>}
        {dates.length>0&&<div style={{fontSize:12,color:"#374151",fontWeight:600,paddingTop:8,borderTop:"1px solid #e5e7eb"}}>
          {dates.length} active day{dates.length!==1?"s":""}:&nbsp;<span style={{fontWeight:400,color:"#6b7280"}}>{dates.map(d=>fdS(d)).join(" · ")}</span>
        </div>}
      </div>
      ...
      <div style={{background:"#f9fafb",border:"1px solid #e5e7eb",borderRadius:10,padding:"14px 16px",marginBottom:16}}>
        <div style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"flex-end",marginBottom:12}}>
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <button onClick={()=>setGG(g=>Math.max(1,g-1))} style={{width:30,height:30,border:"1px solid #d1d5db",borderRadius:6,background:"#fff",cursor:"pointer",fontSize:16,fontWeight:700}}>−</button>
              ...
              <button onClick={()=>setGG(g=>Math.min(MAXG,g+1))} style={{width:30,height:30,border:"1px solid #d1d5db",borderRadius:6,background:"#fff",cursor:"pointer",fontSize:16,fontWeight:700}}>+</button>
            </div>
        ...
        <div style={{borderTop:"1px solid #e5e7eb",paddingTop:10,display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          <div><LB t="Client Name"/><input value={cN} onChange={e=>setCN(e.target.value)} placeholder="e.g. Jón Sigurðsson" style={iS}/></div>
      ...
      {dates.length>0&&<>
        ...
        {tab==="summary"&&<div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr>{[["Date",false],["Type",false],["Meals",false],["Service Fee",true],["Food & Bev",true],["Complexity",true],["Day Total",true]].map(([h,r])=><th key={h as string} style={thS(r as boolean)}>{h}</th>)}</tr></thead>
            <tbody>{days.map((day:any,i:number)=>{
              const c=calcs[i],p:string[]=[];
              if(day.bf)p.push(`Bfast ×${c.bfG}`);
              if(c.effDin&&c.effDin!=="none")p.push(`${(mb[c.effDin]?.label||"").replace("Dinner – ","").replace(" (1-course)","")} ×${c.dinG}${c.frkOnly?" (no svc)":""}`);
              if(day.lType!=="none")p.push(`Lunch ×${c.lG}`);
        ...
        {tab==="days"&&<div>
          <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:12}}>
            {days.map((day:any,i:number)=>{const c=calcs[i],act=sel===day.date;
          ...
          {(()=>{
            ...
            const di=days.findIndex((d:any)=>d.date===sel);if(di<0)return null;
            const day=days[di],c=calcs[di],u=(f:string,v:any)=>upd(day.date,{[f]:v});
              ...
              <div style={{padding:14,display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  ...
                  <div style={{background:"#f9fafb",borderRadius:8,padding:10}}>
                    ...
                    <div style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid #e5e7eb"}}>
                      ...
                      {day.bf&&<div style={{paddingLeft:16,borderLeft:"2px solid #e5e7eb"}}><GIn label="Guests" value={day.bfG} onChange={v=>u("bfG",v)} inh={c.dG} sub/></div>}
                    </div>
                    <div style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid #e5e7eb"}}>
                      ...
                      <div style={{display:"flex",gap:5,marginBottom:4}}>
                        ...
                        {day.dinOv&&<button onClick={()=>upd(day.date,{dinOv:false,dinId:null})} style={{padding:"3px 7px",border:"1px solid #f59e0b",borderRadius:5,background:"#fff",color:"#f59e0b",cursor:"pointer",fontSize:11}}>✕</button>}
                      </div>
                      {c.frkOnly&&<div style={{marginTop:4,background:"#fffbeb",border:"1px solid #fcd34d",borderRadius:5,padding:"6px 8px",fontSize:11,color:"#92400e"}}>Fröken only — no service fee. Flat {ISK(FRKRATE)}/pax.</div>}
                      {c.effDin&&c.effDin!=="none"&&<div style={{paddingLeft:16,borderLeft:"2px solid #e5e7eb",marginTop:6}}><GIn label="Guests" value={day.dinG} onChange={v=>u("dinG",v)} inh={c.dG} sub/></div>}
                    </div>
                    <div style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid #e5e7eb"}}>
                      ...
                      {day.lType!=="none"&&<div style={{paddingLeft:16,borderLeft:"2px solid #e5e7eb",display:"flex",flexDirection:"column",gap:4}}>
                        <GIn label="Guests" value={day.lG} onChange={v=>u("lG",v)} inh={c.dG} sub/>
                        {day.lType==="custom"&&<div><LB t="Food cost excl. VAT & markup" s/><input type="number" min={0} value={day.lBudg} onChange={e=>u("lBudg",e.target.value)} placeholder="e.g. 20000" style={{...iS,width:130}}/></div>}
                      </div>}
                    </div>
                    <div style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid #e5e7eb"}}>
                      ...
                      {day.drkOn&&<div style={{paddingLeft:16,borderLeft:"2px solid #e5e7eb",display:"flex",flexDirection:"column",gap:4}}>
                        <select value={day.drkT} onChange={e=>u("drkT",e.target.value)} style={iS}>{Object.entries(DRK).map(([k,v])=><option key={k} value={k}>{(v as any).label}</option>)}</select>
                        ...
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                          ...
                          <GIn label="Guests" value={day.drkG} onChange={v=>u("drkG",v)} inh={c.dG} sub/>
                        </div>
                      </div>}
                    </div>
                    <div style={{marginBottom:10,paddingBottom:10,borderBottom:"1px solid #e5e7eb"}}>
                ...
                <div style={{background:"#f9fafb",borderRadius:8,padding:12}}>
                  ...
                  :c.isDelivery?<div style={{fontSize:12,color:"#1e40af",padding:"3px 0"}}>No service fee (Meal Delivery / Catering)</div>
                    ...
                    ].map((r:any,i:number)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"3px 0",borderBottom:"1px solid #e5e7eb",fontSize:12}}>
                  ...
                  {c.lines.length>0&&<>
                    ...
                    {c.lines.map((l:any,i:number)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"3px 0",borderBottom:"1px solid #e5e7eb",fontSize:11}}>
                      <span>{l.label} ×{l.pax}{l.isFixed&&<span style={{color:"#9ca3af",fontSize:9}}> (flat)</span>}</span>
        ...
        {tab==="meals"&&<div>
          ...
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>{Object.entries(tots.mMap).map(([k,m]:any)=><tr key={k}>
              <td style={tdS(false)}>{m.label}{m.isFixed&&<span style={{color:"#9ca3af",fontSize:10}}> (flat)</span>}</td>
              <td style={tdS(true)}>{m.days}×</td><td style={tdS(true)}>{m.pax}</td>
        ...
        {tab==="menus"&&<div>
          <div style={{background:"#fffbeb",border:"1px solid #fcd34d",borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:12,color:"#92400e"}}>
            ⚠️ Estimates only — final invoice based on actual cost of goods + 11% VAT + 30% markup. Fröken Selfoss, Staff Dinner and Lunch Box are flat rates — no additional markup.
          </div>
              ...
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                ...
                <tbody>{cm.map((m:any)=>{
                  ...
                  return <tr key={m.id}>
                    ...
                    <td style={{...tdS(true),color:"#6b7280"}}>{isFlat?"Flat incl. VAT":ISK(avat(m.base))}</td>
                    <td style={{...tdS(true),fontWeight:700}}>{isFlat?ISK(m.base)+" (flat)":ISK(afull(m.base))}</td>
                    <td style={{textAlign:"right",padding:"7px 10px"}}>
                      <div style={{display:"flex",gap:5,justifyContent:"flex-end",alignItems:"center"}}>
                        ...
                        {m.del&&<button onClick={()=>setMenus((ms:any[])=>ms.filter((x:any)=>x.id!==m.id))} style={{padding:"2px 7px",border:"1px solid #fca5a5",borderRadius:4,background:"#fff",color:"#dc2626",cursor:"pointer",fontSize:11}}>✕</button>}
                      </div>
                    </td>
                  </tr>;
                })}</tbody>
        ...
        {tab==="policy"&&<div style={{maxWidth:680,fontSize:13,lineHeight:1.8,color:"#374151"}}>
          {[
            {n:"1. Service Fee Structure",c:<><p><strong>Daily Service Fee:</strong> 98,000 ISK per day (incl. 11% VAT). Includes menu planning, sourcing, preparation, cooking, service, and travel within 30 min of Selfoss.</p><p><strong>Public Holidays:</strong> 1.9× the daily rate — Christmas (24–25 Dec), New Year (31 Dec – 1 Jan), National Day (17 Jun), Commerce Day, Good Friday, Easter Sunday, Pentecost.</p><p><strong>Additional Staff:</strong> 58,800 ISK per person per day (60% of full daily rate), regardless of day type. Holiday multiplier applies on public holidays. Required for full bar service. Welcome cocktail and wine pairing can be served by the chef.</p><p style={{fontSize:11,color:"#6b7280",borderLeft:"3px solid #e5e7eb",paddingLeft:10,marginTop:8}}><em>Staff/Host Dinner: Lodge management only — flat 2,900 ISK/pax (incl. VAT), no markup or holiday surcharge.</em></p></>},
            {n:"2. Multi-Day & Non-Consecutive Bookings",c:<><p>Partial days (arrival/departure in a consecutive range) billed at 60% of daily fee (58,800 ISK), minimum 72,000 ISK. Single-day bookings always at full rate, minimum 120,000 ISK.</p><p>For non-consecutive bookings (e.g. private chef on selected days within a longer group stay), each booked day is priced independently. Days not booked incur no charge.</p></>},
            ...
            {n:"4. Minimum Spend",c:<><p>Applies to total day spend (service + food + beverages).</p><ul style={{margin:"4px 0 8px",paddingLeft:20}}><li>Full day: <strong>120,000 ISK</strong></li><li>Partial day: <strong>72,000 ISK</strong></li><li>Fröken Selfoss only: <strong>no minimum</strong></li></ul></>},
            {n:"5. Fröken Selfoss – Partner Restaurant",c:<><p>15% group discount on current set menu. See <a href="https://www.frokenselfoss.is" target="_blank" style={{color:"#2563eb"}}>frokenselfoss.is</a> for current prices. Flat pass-through — no additional VAT or markup.</p><ul style={{margin:"4px 0",paddingLeft:20}}><li>Fröken dinner only: no service fee, no minimum</li><li>Breakfast + Fröken: partial-day service fee for breakfast only</li></ul></>},
            ...
            {n:"7. Optional Add-Ons",c:<><p><strong>On-Site Lunch:</strong> 60,000 ISK service fee + ingredient cost (incl. VAT + 30% markup).</p><p><strong>Takeaway Lunch Box:</strong> 3,800 ISK/pax (flat rate, incl. VAT). Includes main, juice, energy bar and sweet treat, fruit, and water.</p><p><strong>Drinks:</strong> Ingredient cost (incl. VAT) + 30% markup. Welcome cocktail (est. 800 ISK/pax) and wine pairing served by chef. Full bar requires dedicated bartender (58,800 ISK/day).</p></>},
            {n:"8. Menu Complexity",c:<p>Multiple separate menus or strict allergy protocols may incur a surcharge, agreed in advance. Final guest count and dietary requirements due 7 days before the event.</p>},
